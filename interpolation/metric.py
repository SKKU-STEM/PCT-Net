...
def create_window(window_size, channel=1):
    ...
    _2D_window = _1D_window.mm(_1D_window.t()).float().unsqueeze(0).unsqueeze(0).to(torch.device("cpu"))
...
def calculate_ssim(img1, img2, window_size=11, window=None, size_average=True, full=False, val_range=None):
    ...
    if window is None:
        ...
        window = create_window(real_size, channel=channel).to(torch.device("cpu"))
